...
class Timeline:
    def __init__(self, filename):
        ...
        # set up some params
        self.callout_size = (10, 30, 10)  # width, height, increment
    ...
    def create_eras(self, y_era, y_axis, height):
        ...
        for era in eras_data:
            ...
            percent_width0 = (
                t0[0] - self.date0).total_seconds()/self.total_secs
            percent_width1 = (
                t1[0] - self.date0).total_seconds()/self.total_secs
            x0 = int(percent_width0*self.width + 0.5)
            x1 = int(percent_width1*self.width + 0.5)
            rect = self.drawing.add(
                self.drawing.rect((x0, 0), (x1-x0, height)))
            rect.fill(fill, None, 0.15)
            line0 = self.drawing.add(self.drawing.line(
                (x0, 0), (x0, y_axis), stroke=fill, stroke_width=0.5))
            line0.dasharray([5, 5])
            line1 = self.drawing.add(self.drawing.line(
                (x1, 0), (x1, y_axis), stroke=fill, stroke_width=0.5))
            line1.dasharray([5, 5])
            # create horizontal arrows and text
            horz = self.drawing.add(self.drawing.line(
            ...
            self.drawing.add(self.drawing.text(name, insert=(
                0.5*(x0 + x1), y_era - self.text_fudge[1]), stroke='none', fill=fill, font_family="Serif", font_size="6pt", text_anchor="middle"))
    ...
    def get_markers(self, color):
        ...
        if color in self.markers:
            ...
            start_marker.add(self.drawing.path(
                "M6,0 L6,7 L0,3 L6,0", fill=color))
            self.drawing.defs.add(start_marker)
            end_marker = self.drawing.marker(
                insert=(6, 3), size=(10, 10), orient='auto')
            end_marker.add(self.drawing.path(
                "M0,0 L0,7 L6,3 L0,0", fill=color))
    ...
    def create_main_axis(self):
        # draw main line
        self.g_axis.add(self.drawing.line(
            (0, 0), (self.width, 0), stroke=Colors.black, stroke_width=3))
        # add tickmarks
        self.add_axis_label(self.start_date, str(
            self.start_date[0]), tick=True)
        ...
        if 'num_ticks' in self.data:
            ...
            for j in range(1, num_ticks):
                ...
                self.add_axis_label(
                    [tickmark_date], str(tickmark_date), tick=True)
    ...
    def add_axis_label(self, dt, label, **kwargs):
        ...
        if add_tick:
            ...
            self.g_axis.add(self.drawing.line(
                (x, -dy), (x, dy), stroke=stroke, stroke_width=2))
        # add label
        fill = kwargs.get('fill', Colors.gray)
        transform = "rotate(180, %i, 0)" % (x)
        self.g_axis.add(self.drawing.text(label, insert=(x, -2*dy), stroke='none', fill=fill,
                        font_family='Serif', font_size='6pt', text_anchor='end', writing_mode='tb', transform=transform))
        h = self.get_text_metrics('Serif', 6, label)[0] + 2*dy
    ...
    def create_callouts(self):
        ...
        for event_date in sorted_dates:
            ...
            k = 0
            i = len(prev_x) - 1
            left = x - (self.get_text_metrics('Serif', 6, event)
                        [0] + self.callout_size[0] + self.text_fudge[0])
            ...
            self.g_axis.add(self.drawing.path(
                path_data, stroke=event_color, stroke_width=1, fill='none'))
            self.g_axis.add(self.drawing.text(event, insert=(
                x - self.callout_size[0] - self.text_fudge[0], y + self.text_fudge[1]), stroke='none', fill=event_color, font_family='Serif', font_size='6pt', text_anchor='end'))
            self.add_axis_label(event_date, str(
                event_date[0]), tick=False, fill=Colors.black)
            self.g_axis.add(self.drawing.circle(
